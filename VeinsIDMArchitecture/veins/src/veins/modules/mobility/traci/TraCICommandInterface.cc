...
void TraCICommandInterface::Vehicle::stopAt(std::string roadId, double pos, uint8_t laneid, double radius, simtime_t waittime)
{
    ...
    TraCIBuffer buf = connection->query(CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType
            << count << edgeIdT << edgeId << stopPosT << stopPos << stopLaneT << stopLane << durationT << duration);
    ASSERT(buf.eof());
}

std::pair<std::string, double> TraCICommandInterface::Vehicle::getLeader(double lookAhead)
{

    uint8_t variable = VAR_LEADER;
    std::string simId = "sim0";
    uint8_t variableType = TYPE_COMPOUND;
    uint32_t count = 2;
    TraCIBuffer buf = connection->query(CMD_GET_VEHICLE_VARIABLE, TraCIBuffer() << variable << nodeId << TYPE_DOUBLE << lookAhead);
   ...
   uint8_t typeId_resp;
    buf >> typeId_resp;
  //  std::cout << "Type id response is: " << typeId_resp << std::endl;
    ASSERT(typeId_resp == variableType);
    //std::cout  << "This car is: " << nodeId << std::endl;
  //  std::cout << "Remaining values of buffer is: " << buf.str() << std::endl;
    uint32_t compoundLength; buf >> compoundLength; ASSERT(compoundLength == count);
      uint8_t typeId_resp2; buf >> typeId_resp2; ASSERT(typeId_resp2 == TYPE_STRING);
      std::string leaderId; buf >> leaderId;
      uint8_t typeId_resp3; buf>> typeId_resp3; ASSERT(typeId_resp3 == TYPE_DOUBLE);
      double distance; buf >> distance;

    return std::make_pair(leaderId, distance);
}
...
//////mine
int TraCICommandInterface::Road::getLastStepNumberOfCars(){
    return traci->genericGetInt(CMD_GET_EDGE_VARIABLE, roadId, LAST_STEP_VEHICLE_NUMBER, RESPONSE_GET_EDGE_VARIABLE);
}

//////mine
std::list<std::string> TraCICommandInterface::Lane::getLastStepVehicleIds(){
...
double TraCICommandInterface::Lane::getWaitingTime(){
    return traci->genericGetInt(CMD_GET_LANE_VARIABLE, laneId, 0x14, RESPONSE_GET_LANE_VARIABLE);
}
...
bool TraCICommandInterface::removeVehicle(std::string vehicleId){
    TraCIConnection::Result result;

    uint8_t variableId = REMOVE;
    uint8_t variableType = TYPE_BYTE;
    uint8_t reason = REMOVE_VAPORIZED;
    TraCIBuffer buf = connection.query(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << vehicleId << variableType << reason);
    ASSERT(buf.eof());
    return result.success;
}
