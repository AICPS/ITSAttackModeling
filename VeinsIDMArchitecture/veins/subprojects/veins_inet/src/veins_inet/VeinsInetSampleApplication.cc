...
bool VeinsInetSampleApplication::startApplication()
{
    ...
    if (getParentModule()->getIndex() == 0) {
        auto callback = [this]() {
            ...
            // create the payload and fill it with the appropriate contents: here it is chunk length and road ID
            // chunk length is related to inet related properties. roadID is the unique application specific data

            auto payload = makeShared<VeinsInetSampleMessage>();
            timestampPayload(payload);
            payload->setChunkLength(B(100));
            payload->setRoadId(traciVehicle->getRoadId().c_str());
            // create the packet and insert the payload into it.
            // the packet is an "accident" related packet and is an overriden function in veins inet application base
